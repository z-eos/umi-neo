...
sub new_netgrp ($self) {
  ...
  if ( ! exists $p->{dn_to_modify_netgr} ) {
    ...
    my $ngt = $self->ngt($p, \%debug);
    # $self->h_log($ngt);

    # on form submit
    # $self->h_log($p);
    if ( keys %$p > 1 ) { # no data ==> no changes
      my $diff = $self->h_array_diff($ngt->{nisNetgroupTriple}, $ngt->{tuples});
      # $self->h_log($diff);
	...
	push @$changes, add => $add;
      ...
      if ( @{$diff->{removed}} ) {
	push @$delete, nisNetgroupTriple => $diff->{removed};
	push @$changes, delete => $delete;
      }

      if ($changes) {
	# $self->h_log($changes);
	$msg = $ldap->modify($p->{dn_to_modify_netgr}, $changes);
	$self->stash(debug => {$msg->{status} => [ $msg->{message}, $self->h_np($changes) ]});
	undef $ngt;
	$ngt = $self->ngt( {dn_to_modify_netgr => $p->{dn_to_modify_netgr}}, \%debug);
	# $self->h_log($ngt);
      }
    ...
    $self->stash( dn_to_modify_netgr => $p->{dn_to_modify_netgr},
		  cn => $ngt->{cn},
		  tuples_grouped => $ngt->{tuples_grouped},
		  memberUid => $ngt->{tuples_users},
		  hosts => $ngt->{tuples_hosts} );
  }
...
sub ngt {
  ...
  my ($memberUid, $err) = $ldap->all_users({with => 'root_and_ssh'});
  push @{$debug->{$err->{status}}}, $err->{message} if defined $err;
  undef $err;
  my %memberUid_hash = map { $_->[1] => $_->[0] } @$memberUid;
  ($hosts, $err) = $ldap->all_hosts;
  ...
  my $search_arg = { base => $p->{dn_to_modify_netgr}, scope => 'base' };
  my $msg = $ldap->search( $search_arg );
  $self->h_log( $self->h_ldap_err($msg, $search_arg) ) if $msg->code;
  my $entry = $msg->entry;
  $ret->{cn} = $entry->get_value('cn');

  #- STEP1: Build a mapping of `hostname.domain` to `user`s
  $ret->{nisNetgroupTriple} = $entry->get_value('nisNetgroupTriple', asref => 1);
  for my $triple ( @{$ret->{nisNetgroupTriple}} ) {
    ($ret->{hostname}, $ret->{user}, $ret->{domain}) = split /,/, substr($triple,1,-1);
    my $host_domain = "$ret->{hostname}.$ret->{domain}";
    push @{ $ret->{host_to_users}->{$host_domain} }, { user => $ret->{user}, host => $ret->{hostname}, domain => $ret->{domain} };
  }

  #- STEP2: Identify unique sets of users (Cartesian product groups)
  my %grouped;
  for my $host_domain (keys %{$ret->{host_to_users}}) {
    my @users = sort map { $_->{user} } @{ $ret->{host_to_users}->{$host_domain} };
    my $key   = join ',', @users; # Unique signature for this user set
    push @{ $grouped{$key} }, @{ $ret->{host_to_users}->{$host_domain} };
  }

  #- STEP3: Convert to a flat format to be compared with data from LDAP object
  @{$ret->{tuples_grouped}} = map {
  ...
  if ( exists $p->{memberUid} ) {
	...
	@{$p->{hosts}->[$i]};
      @{$ret->{tuples}} = (@{$ret->{tuples}}, @t);
      $i++;
    }
  }

  my @keys = sort keys %grouped;
  @{$ret->{tuples_users}} = map {
    my @all = map { $_->{user} } @{ $grouped{$_} };
    my %seen;
    [ grep { !$seen{$_}++ } @all ];
  } @keys;

  @{$ret->{tuples_hosts}} = map {
