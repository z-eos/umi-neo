...
=cut
  ...
  $app->helper( h_ldif_header => sub {
		  my  ($self) = @_;
		  return $self->
		    h_decode_text( sprintf("### Generated by %s (%s %s) on %s\n",
					   $self->session('uid'),
					   $self->h_translit( $self->session('user_obj')->{givenname} ),
					   $self->h_translit( $self->session('user_obj')->{sn} ),
